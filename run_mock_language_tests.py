"""
Run language efficiency tests with mock data.
"""
        ...
        for prompt_type in prompt_types:
            for model in models:
                ...
                mock_result = {
                    ...
                    "input_tokens": 150 + (100 if prompt_type == "chinese" else 0),
                    "output_tokens": 300 + (150 if prompt_type == "chinese" else 0),
                    "total_tokens": 450 + (250 if prompt_type == "chinese" else 0),
                    "response_time": 2.5,
                    "response_text": f"Mock response for {problem['id']} using {prompt_type} prompt on {model}",
                    ...
                    "response_compression_ratio": 1.2 if prompt_type == "english" else 1.8,
                    "response_chars_per_token": 3.5 if prompt_type == "english" else 5.2,
                    "response_chinese_char_count": 0 if prompt_type == "english" else 800,
                    "response_chinese_ratio": 0.0 if prompt_type == "english" else 0.65,
                    "response_chinese_info_density": 0.0 if prompt_type == "english" else 2.3,
                    "response_chinese_chars_per_token": 0.0 if prompt_type == "english" else 2.8
                }
...
def main():
    ...
    print(f"Running language efficiency tests with models: {', '.join(models)}")
    
    # Initialize test runner with models
    ...
    results = generate_mock_results(test_problems, prompt_types, models)
    
    # Add prompt_type column if not present
    for result in results:
        if "prompt_type" not in result:
            result["prompt_type"] = "english"  # Default to English
    ...
    print("Check the 'experiment_results' and 'reports' directories for results and visualizations.")
